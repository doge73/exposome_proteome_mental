...
#######CTCT for one continuous and one binary variables############################################

imputated_twin_CTCT$alc_ch_cr1<-as.factor(imputated_twin_CTCT$alc_ch_cr1)
imputated_twin_CTCT$alc_ch_cr2<-as.factor(imputated_twin_CTCT$alc_ch_cr2)

# Select Continuous Variables
vars <- c('O95445') # list of continuous variables names
...
vars <- c('alc_ch_cr') # list of ordinal variables names
...
wtquant <- quantile(ordData[,c('alc_ch_cr1','alc_ch_cr2')],(0:(nth+1))/(nth+1),na.rm=TRUE)
# Select Variables for Analysis
vars <- c('O95445','alc_ch_cr') # list of variables names
...
mzDataF <- cbind(mzData[,conVars],mxFactor( x=mzData[,ordVars], levels=c(1:2)) )
dzDataF <- cbind(dzData[,conVars],mxFactor( x=dzData[,ordVars], levels=c(1:2)) )
...
thinMZ <- mxMatrix( type="Full", nrow=nth, ncol=ntvo, free=TRUE, values=svTh, lbound=lbTh, labels=c('thre1'), name="thinMZ" )
thinDZ <- mxMatrix( type="Full", nrow=nth, ncol=ntvo, free=TRUE, values=svTh, lbound=lbTh, labels=c('thre1'), name="thinDZ" )
